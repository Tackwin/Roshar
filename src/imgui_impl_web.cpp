...
void FunImGui::initGraphics()
{
	...
	shader_errors(vertexHandle);
	GL_CHECKED(glCompileShader(fragmentHandle));
	shader_errors(fragmentHandle);
...
void shader_errors(int x) {
	...
	if (!ok) {
		printf("Shader error\n");
		int infoLen = 0;
		glGetShaderiv(x, GL_INFO_LOG_LENGTH, &infoLen);
		if (infoLen > 1)
		{
			char* infoLog = new char[infoLen + 1];
			glGetShaderInfoLog(x, infoLen, NULL, infoLog);
